...
let substitution_in_place (subst : SSubst.t) (x : t) : unit =
	...
	Var.Set.iter (fun v ->
		...
		if (le <> s_le) then put x v s_le) symbolics)

(** Returns the set containing all the vars occurring in --x-- *)
let vars (x : t) : SS.t =
	fold x (fun x le ac -> 
		SS.union ac (SS.add x (Expr.vars le))) SS.empty
...
let assertions (x : t) : Formula.t list =
	fold x (fun x le (assertions : Formula.t list) -> (Eq (PVar x, le)) :: assertions) []
