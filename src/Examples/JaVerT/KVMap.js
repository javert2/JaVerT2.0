/*
    ...
    [def1] (kvs == -{ }-) * (keys == -{ }-),
    [def2] (kvs == -u- (-{ {{ #k, #v }} }-, #rkvs)) * (keys == -u- (-{ #k }-, #rkeys)) *
                (! (#k --e-- #rkeys)) * 
...
/**
	...
	@pre     (k == #k) * Map(this, #mp, #kvs, #keys) * MapProto(#mp) * InvalidKey(#k) * GlobalObject() * ObjectPrototype() * BI_ErrorObject()
	@posterr Map(this, #mp, #kvs, #keys) * MapProto(#mp) * ErrorObjectWithMessage(ret, "Invalid Key") * GlobalObject() * ObjectPrototype() * BI_ErrorObject()
...
Map.prototype.get = function (k) {
	...
	if (this.validKey(k)) {
		/* @tactic if (#k -e- #keys) then { unfold KVPairs(#c, #kvs, #keys) [def2 with (#k := #k) and (#v := #v)] } */
...
/**
	...
	@pre     (k == #k) * (v == #v) * Map(this, #mp, #kvs, #keys) * MapProto(#mp) * InvalidKey(#k) * GlobalObject() * ObjectPrototype() * BI_ErrorObject()
	@posterr Map(this, #mp, #kvs, #keys) * MapProto(#mp) * ErrorObjectWithMessage(ret, "Invalid Key") * GlobalObject() * ObjectPrototype() * BI_ErrorObject()
...
Map.prototype.put = function (k, v) {
	...
	if (this.validKey(k)) { 
		/* @tactic if (#k -e- #keys) then { unfold KVPairs(#c, #kvs, #keys) [def2 with (#k := #k) and (#v := #w) and (#rkvs := #rkvs)] } */
		this._contents[k] = v; 
		/* @tactic if (#k -e- #keys) 
