...
let star (asrts : t list) : t =
  List.fold_left
    (fun ac a ->
      if (not (a = Emp))
        then (if (ac = Emp) then a else Star (a, ac))
        else ac)
    Emp
    asrts 


let rec js2jsil_pure
...
let rec js2jsil
  ...
  let compile_pred s les = 
    ...
    if (s = funobj_pred_name)
      then (
        (match les with
        | [ _; Lit (String fid); le_sc; _; _ ] -> Asrt.Star (a', f (SChain (fid, le_sc)))
        | _ -> a')
      ) else a' in 
  ...
  | Pred (name, [loc; Lit (String fid); sch; args_len; fproto]) 
    ...
    let a_len = Asrt.Pure (Eq (Lit (Num ((float_of_int (len-1)))), UnOp (LstLen, fe sch))) in 
    let a_lg  = Asrt.Pure (Eq (Lit (Loc JS2JSIL_Constants.locGlobName), Expr.BinOp (fe sch, LstNth, Expr.Lit (Num (float_of_int 0))))) in
    let a_pred = compile_pred name [loc; Lit (String fid); sch; args_len; fproto] in 
  ...
  | VarSChain (fid, x, le_x, le_sc) ->
    ...
    let a'  =
      (match i with
      | None | Some 0 ->
        let desc = Expr.EList [ Expr.Lit (String "d"); (fe le_x); Expr.Lit (Bool true); Expr.Lit (Bool true); Expr.Lit (Bool false) ] in
        Asrt.PointsTo (Expr.Lit (Loc JS2JSIL_Constants.locGlobName), Expr.Lit (String x), desc)
      | Some i ->
        let le_er = Expr.BinOp (fe le_sc, LstNth, Expr.Lit (Num (float_of_int i))) in
        let not_lg = Asrt.Pure (Formula.Not (Formula.Eq (le_er, Expr.Lit (Loc locGlobName)))) in 
        Asrt.star [ not_lg; Asrt.PointsTo (le_er, Expr.Lit (String x), fe le_x) ] ) in
  ...
  | OSChains (fid1, le_sc1, fid2, le_sc2) ->
    ...
    let f j      = Asrt.Pure (Formula.Eq (Expr.BinOp (le_sc1', LstNth, Expr.Lit (Num (float_of_int j))), Expr.BinOp (le_sc2', LstNth, Lit (Num (float_of_int j))))) in
  ...
  | Closure (var_les, fid_sc_les) ->
    ...
    let asrt_vars = List.map (fun (x, le_x) -> VarSChain (fid_1, x, le_x, le_sc_1)) var_les in
    let asrt_scs  = List.map (fun (fid_j, le_sc_j) -> OSChains (fid_j, le_sc_j, fid_1, le_sc_1)) rest_fid_sc_les in
    ...
    let asrt_lens : t list = List.map (fun (x, _, z) -> Pure (Eq (UnOp (LstLen, z), Lit (Num (float_of_int x))))) fsclens in 
...
let errors_assertion () =
  Asrt.Star (
    Pred (type_error_pred_name,   [ (PVar JS2JSIL_Constants.var_te) ]),
    Pred (syntax_error_pred_name, [ (PVar JS2JSIL_Constants.var_se) ])
  )
...
let rec js2jsil_tactic
  ...
  let a'  = js2jsil (Some fid) cc_tbl vis_tbl fun_tbl (Some (Expr.LVar scope_var)) a in
